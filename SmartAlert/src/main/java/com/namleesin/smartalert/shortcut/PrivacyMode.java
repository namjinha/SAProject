...
public class PrivacyMode extends Activity{
    ...
    public static void changePrivacyMode(Context context)
    {
        SharedPreferences pref = context.getSharedPreferences("pref", MODE_PRIVATE);
        int mode = pref.getInt(PrivacyMode.PREF_PRIVACY_MODE, -1);
        if(mode == -1)
            mode = PrivacyMode.PRIVACY_MODE_OFF;
    ...
    public static void addPrivacyModeShortcut(Context context)
    {
        ...
        addIntent.putExtra( Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
                Intent.ShortcutIconResource.fromContext(context, R.drawable.privacy_off_xhdpi));
